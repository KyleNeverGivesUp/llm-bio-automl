...
| Version | v1.1 |
| Status | Draft (reviewed, ready to build) |
...
|---|---|---|---|
| **Designer** (`src/agent/menu_designer.py`) | ✅ LLM | ✅ **live** in `scripts/run_auto.py` | proposes candidates from the registries; deterministic fallback |
| **Tuner** (`src/agent/menu_tuner.py`) | ✅ LLM | ✅ **live** in `run_auto.py --tune-top` | proposes hyperparameters for the top bases; deterministic fallback. Both optimize the **Set-1 judge** |
| **Manager** (`run_auto.py`) | control flow | ✅ live | drives the Designer/Tuner loop, budget, resume, never-regress (the loop logic itself is deterministic) |
...
| **Retrieval** (`src/agent/retrieval_agent.py`) | LLM (legacy) | ❌ dormant | low value with a small fixed representation menu; not connected |
| Legacy `designer_agent.py` / `manager_agent.py` / `tuner_agent.py` | mixed | ❌ superseded | tied to the old `PlanSpec`/runner; replaced by the clean `MenuPlan`-based modules above |

So today **two** components are LLM-driven and running (Designer, Tuner); the Selector is intentionally deterministic; Retrieval and the legacy agents are dormant.
...
> **Status update (2026-06-22):** M1–M3 done, M4 skipped. **The model side is at its ceiling (~0.62 on the Set-1 judge vs ~0.54 leaderboard top).** The remaining lever is **data, not method** → **M5 Data side is now the priority.**
...
| **M6 Research-grounded playbook** (current) | **deep research → grounded implementation** | Read the documented top solutions (via deep research, see §7.1 + `research/worth_trying_MERGED.md`) and adopt their winning techniques, each judged on Set 1: **CheMeleon graph model (frozen + fine-tuned)** → **multitask Chemprop** (primary + counter-assay + targeted PXR external) → **counter-assay weighting** → **Mordred/FCFP4-count** features → honest calibration. **End-game:** Set-1 fold-in + mandatory method report. | each adopted technique lowers (or is dropped on) the Set-1 judge; a frozen recipe + report by July 1 | ⏳ **current** — CheMeleon done (0.6266→**0.6135**); isotonic tried (no help); **multitask Chemprop next** (the documented 0.689→0.60 lever). **Top-5 achievable on public data** (most top-5 used no proprietary data); gap = execution depth |
