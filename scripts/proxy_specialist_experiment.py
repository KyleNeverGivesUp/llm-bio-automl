...
# --------------------------------------------------------------------------- #
def train_specialist(proxy_csv: Path, out_dir: Path, epochs: int, batch: int, accel: str,
                     seed: int = 0) -> str:
    """Fine-tune CheMeleon on the proxy labels. Single target — this model only needs to be
    good at recognising weak molecules; the main ensemble owns the rest. `seed` varies the
    weight init + data shuffle so we can run multiple seeds and report mean/std (a 0.005 gain
    is within CheMeleon's run-to-run noise, so a single run cannot confirm it)."""
...
def _score(names: pd.Series, pred: np.ndarray, labels: pd.DataFrame, tag: str, quiet: bool = False) -> float:
...
# --------------------------------------------------------------------------- #
def run_one_seed(seed, args, test, p1, p2, cal, quiet):
    """Train a specialist at this seed, pick the blend weight on Set-1, return the Set-2 blend RAE.
    Set-2 is only scored, never used to choose anything."""
    out = args.out / f"seed_{seed}"
    out.mkdir(parents=True, exist_ok=True)
    spec_csv = out / "test_specialist.csv"
    if not args.skip_train:
        ...
        if args.include_real:                       # proxy (mostly weak) + the real dose-response labels
            ...
            train_csv = out / "_train_pool.csv"
            pool.to_csv(train_csv, index=False)
            if not quiet:
                print(f"[data] proxy {len(pool) - len(real)} + real {len(real)} = {len(pool)} molecules")
        test[[SMILES]].to_csv(out / "_test_in.csv", index=False)
        ckpt = train_specialist(train_csv, out / "ckpt", args.epochs, args.batch_size, args.accelerator, seed)
        spec = predict(ckpt, out / "_test_in.csv", spec_csv, args.accelerator)
    ...
    # pick blend weight on Set-1 (dev); Set-2 untouched
    best_w, best_r = 0.0, float("inf")
    for w in np.arange(0.0, 1.01, 0.1):
        r = _score(test[NAME], np.where(fires, w * spec + (1 - w) * cal, cal), p1, f"w={w:.1f}", quiet)
        if r < best_r:
            best_w, best_r = float(w), r
    blended = np.where(fires, best_w * spec + (1 - best_w) * cal, cal)
    set2_rae = _score(test[NAME], blended, p2, f"seed {seed}: blend (w={best_w:.1f})", quiet=True)
    print(f"[seed {seed}] gate fired {int(fires.sum())}/{len(spec)} | w={best_w:.1f} "
          f"(Set-1 {best_r:.4f}) | Set-2 blend RAE = {set2_rae:.4f}")
    return set2_rae, spec, blended
...
def main() -> None:
    ...
    ap.add_argument("--seeds", default="0",
                    help="comma-separated seeds, e.g. '0,1,2'. Multiple seeds -> mean/std, so we can "
                         "tell a real gain from CheMeleon's run-to-run noise (~0.005).")
    ...
    # the main ensemble, variance-matched on its own OOF (deterministic, seed-independent)
    ...
    cal = a * raw + b
    base_rae = _score(test[NAME], raw, p2, "baseline", quiet=True)
    cal_rae = _score(test[NAME], cal, p2, "calibration", quiet=True)
    print(f"[calibration] new = {a:.3f}*old + ({b:.3f})  |  baseline Set-2 {base_rae:.4f} -> calibration {cal_rae:.4f}")

    # per-seed specialist blend
    print(f"\n=== proxy weak-specialist blend, seeds {seeds} ===")
    results = [run_one_seed(s, args, test, p1, p2, cal, quiet=True) for s in seeds]
    blend_raes = np.array([r[0] for r in results])

    print(f"\n=== Set-2 (260 blind, final scorer) ===")
    print(f"  0. baseline                         {base_rae:.4f}")
    print(f"  1. + calibration                    {cal_rae:.4f}")
    if len(seeds) == 1:
        print(f"  2. + proxy blend (seed {seeds[0]})          {blend_raes[0]:.4f}")
    else:
        print(f"  2. + proxy blend ({len(seeds)} seeds)         "
              f"{blend_raes.mean():.4f} ± {blend_raes.std():.4f}   (per-seed {np.round(blend_raes, 4).tolist()})")
        print(f"     -> calibration {cal_rae:.4f} vs blend mean {blend_raes.mean():.4f}: "
              f"gain {cal_rae - blend_raes.mean():+.4f}, "
              f"{'REAL (mean beats calibration by > std)' if cal_rae - blend_raes.mean() > blend_raes.std() else 'within noise'}")
    print("\n  reference: #2 (best public-data) 0.5676 | rank-12 0.586 | #1 (proprietary) 0.5631")

    # bins on the first seed's blend, to show where the correction landed
    bins(test[NAME], cal, p2, "before blend")
    bins(test[NAME], results[0][2], p2, f"after blend (seed {seeds[0]})")
