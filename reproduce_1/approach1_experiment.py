...
def train_fold(train_csv: Path, out_dir: Path, epochs: int, batch: int, accel: str, workers: int) -> str:
    subprocess.run(
        ["chemprop", "train", "-i", str(train_csv), "-s", SMILES, "--target-columns", *TARGETS,
...
def predict(ckpt: str, in_csv: Path, out_csv: Path, accel: str, workers: int) -> np.ndarray:
    subprocess.run(
